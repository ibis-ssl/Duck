...
- `Tracked` 表示でトラッカーの統合結果、キック・接触・競技場の状態、送信関連カウンタを確認する
- 複数の設定組を定義し、UI または API から実行中の設定組を切り替える
...
- `Tracked` モードの `Profile Control` から定義済みの設定組を選べます
- 切り替えを要求すると有効な設定組が切り替わり、古い追跡フレームは一度消去されます
- `VisionReceiver:Profiles` に同名の設定組がある場合は、受信処理のマルチキャスト用の通信アドレス、通信ポート、使用する IPv4 の通信アドレスもその設定組に追従します
- 新しいパケットが確定すると、新しい設定組に基づく追跡フレームが再表示されます
...
- 上部の再生位置操作部をドラッグすると、選択中の再生時点が連続的に切り替わります
- 左側の再生時系列でログ行を時系列にスクロールできます
- 受信記録に付随する補助ファイルと同じ基本名の `*.render-snapshots.jsonl.gz` がある場合は、選択行の未加工入力と追跡結果を競技フィールドに描画できます
...
- 受信記録の診断ログを選ぶと、`Settings` から受信記録の付随情報に保存された定義済みの設定組と解決済みの設定を確認できます
- 受信記録の診断ログとトラッカーパケットのスナップショット補助ファイルが揃っている場合は、折り畳み可能な `Tracker Comparison` 表示領域で自前トラッカーと外部トラッカーの差分を確認できます
...
- `Tracker Comparison` はトラッカーパケットのスナップショット補助ファイルと対応付け用の補助ファイルをログ選択時に軽量な索引へ変換し、統合した再生時系列も同時に構築します。再生位置や再生時点を変更しても同じ補助ファイルを再読込しません。100MB を超える補助ファイルでも、再生位置や再生時点を変えるたびの I/O と解析量は補助ファイルのサイズに比例しません。
...
- 表示領域には補助ファイルの状態、対応付けの状態、記録 / 省略 / エラー件数、選択中の再生時系列の索引 / 時刻、保持中の診断行 / 描画内容、比較状態、対応付け規則、表示元の役割 / 表示名、表示元の識別子 / 送信元の接続先、スナップショット側の追跡フレームの番号、自前 / 対応付け済みスナップショットの時刻、時刻差、ボール / ロボット、未加工受信データの復元状態が表示されます。

## API

設定組の切り替えは HTTP API からも要求できます。既定の起動設定では `UseHttpsRedirection()` が有効なため、通常は HTTPS の接続先を使ってください。
...
- 存在しない設定組名を指定すると `Tracker:Profiles` の解決に失敗し、現状実装では 4xx ではなくサーバーエラーになります
...
| `Profiles` | 設定組ごとの受信処理の上書き設定です。`Tracker` 側の有効な設定組名と同名の項目があれば、起動時と実行中の設定組切り替え後にその受信設定へ追従します。 |
...
受信記録を開始すると、`<prefix>-<timestamp>-<guid>` という CaptureOn の記録単位フォルダを作り、その中に同じ基本名で次の補助ファイルも作成します。

- `<prefix>-<timestamp>-<guid>.jsonl.gz`: パケット受信記録の本体
- `<prefix>-<timestamp>-<guid>.metadata.json`: 受信記録時の `Tracker` 設定と解決済みの設定組
- `<prefix>-<timestamp>-<guid>.tracker-diagnostics.log`: 受信記録と対応するトラッカー診断ログ。`Tracker:Diagnostics:FilePath` が指定されていても、受信記録が有効なときは補助ファイルとして同時に出力します。
- `<prefix>-<timestamp>-<guid>.render-snapshots.jsonl.gz`: 再生時系列と逆方向の再生位置移動に使う描画スナップショット。追跡エンジンの内部状態ではなく、確定済みの `TrackerFrame` だけを保存します。
- `tracker-packet-snapshots.jsonl`: CaptureOn 中に `Tracker:Receive:Enabled=true` の受信処理が受信した公式形式のトラッカーパケットのスナップショット補助ファイルです。
- `tracker-snapshot-alignment.jsonl`: CaptureOn 中の診断記録、描画スナップショット、トラッカーの表示元スナップショットを、1 回の記録の再生時系列上で対応付ける補助ファイルです。外部トラッカーの `TrackedFrame.timestamp` が自前トラッカーと別の時刻系でも、`receivedAt` と記録開始からの相対時間を使って `/diagnostics` の `Field source` と比較を再生できます。保存形式の版 2 では診断ログ行単位ではなく、最も更新が速い表示元の周期に合わせた再生時系列の記録を保存し、同じ `Vision` / 描画記録を複数の高速なトラッカー記録から参照できます。版 1 との互換は持たず、ログ選択時の索引構築と再生位置移動時の高速参照を優先します。
...
`metadata.json` には有効な設定組名だけではなく、`Tracker:Profiles` 配下の設定組の値と、実行時の上書き適用後に解決した設定も保存します。CaptureOn 比較ログがある場合は、`SessionFolder`、`PacketPath`、`DiagnosticsLogPath`、`RenderSnapshotPath`、`TrackerSnapshotSidecarPath`、`TrackerSnapshotAlignmentPath`、`TrackerSnapshotLog`、`TrackerSnapshotAlignmentLog`、`TrackerSnapshotSources` もここから辿ります。

| キー | 意味 |
| --- | --- |
| `Enabled` | 起動時のパケット受信記録の初期値です。起動後は画面の `Capture On/Off` ボタンで切り替えできます。 |
| `DirectoryPath` | 受信記録ファイルの出力ディレクトリです。相対ファイルパスは実行ファイルのディレクトリから解決します。 |
| `FilePrefix` | 受信記録ファイル名の接頭辞です。実際のファイル名は `<prefix>-<timestamp>-<guid>.jsonl.gz` になります。 |
...
現在の `appsettings.json` では起動時の受信記録は無効ですが、画面で `Capture On` にした後はパケットごとに書き出します。
...
保存された受信記録は `Tracker.CaptureReplay` で再生・分析できます。通常のユーザー確認は `/diagnostics` の `Tracker Comparison` 表示領域を主経路にし、この CLI はエージェント / 自動検証 / 回帰調査で同じ記録単位を再現するために残します。詳細は [CaptureReplay の利用手順](../Tracker.CaptureReplay/README.md) を参照してください。
...
CaptureOn 比較ログを CLI で検証する場合は、`--capture` に記録単位のフォルダまたはその中の `*.jsonl.gz` を渡します。フォルダを渡した場合、CLI は同じフォルダの `*.metadata.json` から受信記録と解決済みのトラッカー設定を読み取ります。再生には記録時点で解決済みのトラッカー設定を使い、付随情報に記録された相対ファイルパスから `tracker-packet-snapshots.jsonl` と `tracker-snapshot-alignment.jsonl` も解決します。出力に `trackerSnapshot ... rawPayloadRestored=True` と `trackerComparison ... rule=saved-session-alignment ...` が出れば、保存時対応表に基づく比較まで読み取れています。対応付けがない既存記録では `legacy-nearest-timestamp` または非対応状態を確認してください。

未加工の映像入力に対して自前トラッカーが遅れて見える場合は、受信記録ファイルを手作業で読む代わりに `--analyze-latency` を使います。これは未加工の検出結果の受信周期と、再生後に自前トラッカーの追跡フレームが確定されるまでの受信記録上の遅れを出力します。付随情報由来のスナップショット行が多い記録では `--skip-tracker-snapshots` と `--max-latency-frames` で出力量を絞れます。
...
ER-Force の設定組は同じリポジトリの `docker/dev/README.md` に従います。`Tracker.DebugHost` 側は `Tracker:Receive:Enabled=true` とし、外部トラッカーが送るマルチキャスト接続先と `Tracker:Receive:MulticastAddress` / `Port` / `InterfaceAddress` が合っていることを確認してから CaptureOn します。検証後の停止は同じディレクトリで `./scripts/docker-dev.sh down` を使います。
...
- `--settings <file>`: `Tracker.DebugHost/appsettings.json` 形式、または受信記録の `metadata.json` 形式からトラッカー設定を読み込みます。`Tracker.DebugHost/appsettings.json` 形式では `Tracker:RuntimeOverrides` も設定組へ反映します。

`--settings` はトラッカー設定の解決にも使われます。CaptureOn の付随情報を渡す通常経路では、記録時に保存済みの解決済み設定と補助ファイルの相対ファイルパスを使うため、当時の設定組・上書き設定・スナップショット補助ファイルをまとめて再現できます。手元の `Tracker.DebugHost/appsettings.json` を渡すのは、受信記録の付随情報がない古い記録を現在設定で再評価したい場合や、意図的に別設定で再生したい場合に限ります。その場合、付随情報から辿る `tracker-packet-snapshots.jsonl` は自動解決されないため、CaptureOn 比較ログの確認手順としては付随情報を優先してください。手書きの付随情報を作る場合は、受信記録と同じ記録単位のフォルダを基準に `PacketPath`、`MetadataPath`、`DiagnosticsLogPath`、`RenderSnapshotPath`、`TrackerSnapshotSidecarPath`、`TrackerSnapshotLog` を矛盾なく入れてください。
...
1. `Tracker:Receive:Enabled=true` にし、受信処理が監視する接続先を確認します。`Tracker:Receive:MulticastAddress` / `Port` が未指定なら起動時に解決した自前トラッカーの送信先を使います。既定の `sim` 設定組は `224.5.23.2:11010`、`default` 設定組は `224.5.23.2:10010` です。外部トラッカーが別の接続先に送信している場合は `Tracker:Receive:MulticastAddress` / `Port` を明示します。ネットワーク接続が複数ある環境では `Tracker:Receive:InterfaceAddress` を明示します。
2. `Tracker.DebugHost` を起動し、画面で `Capture On` にしてから SSL-Vision パケットと公式形式のトラッカーパケットを流します。`Tracker:Receive:Enabled=false` のままではトラッカー受信処理が起動しないため、受信記録と診断ログは残ってもトラッカーパケットのスナップショット補助ファイルは増えません。
...
4. `/diagnostics` を開き、同じ記録単位フォルダの `*.tracker-diagnostics.log` を選びます。統合した再生時系列、再生位置操作部、`Play` / `Fast Forward` / `Stop` の再生ボタン、速度選択タブ（`等倍速`、`4x`、`16x`、`64x`）、左右の `Field source` 選択、`Settings` の解決済み設定を確認します。ER-FORCE などトラッカー表示元が `Vision` より速い記録では、再生時系列に高速なトラッカー時点が含まれ、`Vision` / 描画結果の `Field` が選択時点以前の最新結果を保持することも確認します。`Play` の等倍速再生は毎秒 30 回相当の表示更新で実時間の経過に追従するため、高頻度な再生時点の中間表示を省略する場合があります。
...
7. 報告書には、選択中の再生時系列の索引 / 時刻、`Play` の表示更新で到達した時刻、保持中の診断行 / 描画内容、競技フィールドの表示モード、`Layer A` / `Layer B` の `Field source` と可視性、表示元の絞り込み、補助ファイル / 対応付け / 記録の状態、記録 / 省略 / エラー件数、表示元の役割 / 表示名 / 識別子 / 送信元接続先、同率時の選択規則、スナップショット側の追跡フレームの番号、自前側 / 対応付け側の時刻、時刻差、ボール / ロボット、未加工受信データの表示を残します。UI の未加工受信データは `Restored` が `rawPayloadRestored=true`、`Missing` が `rawPayloadRestored=false` を表します。`Play` で表示されなかった中間の再生時点も、再生位置操作部や比較から選択できることを必要に応じて確認します。
...
- `SnapshotMetadataMissing`: 付随情報にトラッカーのスナップショットのログ情報がありません。CaptureOn 比較ログ導入前の受信記録ではこの状態になり得ます。
...
受信処理の設定組上書きです。未指定項目は最上位の `VisionReceiver` の値を引き継ぎます。

| キー | 意味 |
| --- | --- |
| `MulticastAddress` | 設定組切り替え後に参加するマルチキャストグループの通信アドレスです。 |
| `Port` | 設定組切り替え後にソケットを割り当て、受信に使う UDP の通信ポートです。 |
| `InterfaceAddress` | 設定組切り替え後にマルチキャスト参加へ使う IPv4 の通信アドレスです。 |
...
| `SourceName` | トラッカーパケットの表示元名です。設定組ごとの送信設定と合わせてパケット生成処理に渡されます。 |
| `Uuid` | トラッカーパケットの UUID です。受信側で表示元の識別に使う値です。 |
| `ActiveProfileName` | 起動時に使う設定組名です。`Tracker:Profiles` に存在する必要があります。 |
| `Diagnostics` | トラッカーの未加工入力 / 追跡結果の診断ログ出力設定です。 |
| `Receive` | CaptureOn 比較ログ用に公式形式のトラッカーパケットを受信する設定です。既定は無効です。 |
| `RuntimeOverrides` | 起動時に有効な設定組へ上書きする任意の設定群です。設定組の定義を変えずに、一時的な送信設定 / トラッカー調整値を差し込む用途です。 |
| `Profiles` | 設定組ごとの送信 / 追跡エンジン / 調整値の設定です。UI と API の設定組切り替え対象にもなります。 |

### `Tracker:Receive`

CaptureOn 比較ログ用のトラッカーパケット受信設定です。`Enabled=true` のときだけ `TrackerConnectionLib` の受信処理が起動します。`MulticastAddress` / `Port` が未指定なら、起動時に有効な設定組と `Tracker:RuntimeOverrides:Publish` から解決した自前トラッカーの送信先を監視します。`MulticastAddress` / `Port` を明示した場合は、受信処理独自の接続先を監視します。接続先の解決は起動時固定で、実行時の設定組切り替え後に受信ソケットは再構成されません。受信した `TrackerWrapperPacket` は CaptureOn 中だけ `tracker-packet-snapshots.jsonl` へ保存され、Capture Off 中は追記しません。
...
トラッカーの調査用診断ログ設定です。診断ログは常に出力され、`FilePath` が `null` の場合は `VisionReceiver:PacketCapture:DirectoryPath` 配下に起動ごとの `tracker-diagnostics-<timestamp>-<guid>.log` を作成します。パケットの受信記録が有効なときは、受信記録に付随する `*.tracker-diagnostics.log` にも同時に出力します。
...
設定組の基本設定に対する「上書き」です。指定しない項目は設定組側の値をそのまま使います。
...
| `RobotTracker.*` | ロボット追跡の調整値を有効な設定組に対して上書きします。 |
| `BallTracker.*` | ボール追跡の調整値を有効な設定組に対して上書きします。 |
| `KickDetector.*` | キック判定の調整値を有効な設定組に対して上書きします。 |
...
| `Port` | 公式形式のトラッカーパケットの送信先の通信ポートです。設定組ごとに切り替えられます。 |
...
| `GeometryResetFieldLengthThresholdMm` | mm | 競技場の長さの変化を競技場形状の初期化とみなす閾値です。 |
| `GeometryResetFieldWidthThresholdMm` | mm | 競技場の幅の変化を競技場形状の初期化とみなす閾値です。 |
...
競技場形状の初期化が起きると適用待ち状態が切り替わり、旧形状を前提とした追跡フレームは破棄されます。
...
- `default` と `fast` のように複数の設定組を置けます
- 設定組の切り替えでは、送信先の通信ポートだけでなく、追跡エンジン、ロボット追跡、ボール追跡、キック判定の調整値もまとめて切り替えます
- 同一の設定組名でも実行時の上書き設定が違えば別設定として適用されます
...
- `Tracker:ActiveProfileName` や設定組の切り替え先は、必ず `Tracker:Profiles` に定義した名前にしてください
- マルチキャスト受信に失敗する場合は `VisionReceiver:InterfaceAddress` の明示指定を優先してください
- 設定組切り替え API は未知の設定組名に対して 4xx を返さずサーバーエラーになるため、呼び出し側で事前に設定組一覧を一致させてください
