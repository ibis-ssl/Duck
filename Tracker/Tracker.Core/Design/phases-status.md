...
| verification | done | `TRACKER-018` と `TRACKER-019` に加え、`TRACKER-024` が完了し、Kalman 標準準拠後の build/test/review 証跡が reports に存在する。 |
| review | done | `TRACKER-020` に加え、Kalman 標準準拠後の review 結果が記録され、致命的な指摘が残っていない。 |
