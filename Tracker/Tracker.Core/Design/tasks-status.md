...
- Status: in_progress
- TDD Entry: `Tracker/Tracker.Tests/TrackerConnectionLibThirdPartyTrackerTests.cs` に 3rdparty tracker packet 受信・識別の failing test を追加済み。focused test は現在の production implementation 不足で失敗しており、次作業は実装で test を通す。
- Size: small
...
- Exit Criteria:
  ...
  - 実装は未着手。次作業は `MultiTrackerManager<TrackerPacketAdapter>` 側で self除外、remote endpoint / receivedAt 保持、source identity 単位の最新状態保持を実装し、focused test を通す
