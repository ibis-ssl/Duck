...
- ID: TRACKER-011
- Title: ball left field metadata を実装する
- Phase: engine
- Status: in_progress
- Size: medium
- Dependencies: TRACKER-007, TRACKER-009 が完了していること
- Exit Criteria:
  - `BallLeftFieldState` と `BallLeftField` event が生成され、関連契約テストが通る。
...
| TRACKER-011 | ball left field metadata を実装する | engine | in_progress | TRACKER-007, TRACKER-009 | `BallLeftFieldState` と `BallLeftField` event が生成され、関連契約テストが通る。 |
