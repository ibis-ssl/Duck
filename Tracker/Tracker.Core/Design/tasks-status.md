...
- ID: TRACKER-042
- Title: 全 tracker 保存 contract へテストを修正する
- Phase: comparison-logging
- Status: in_progress
- TDD Entry: `TrackerConnectionLibAllTrackerSnapshotContractTests.cs` で self / external / unknown を保存対象にする all tracker contract を作成済み。focused test は現 production の self 除外と metadata 未実装により失敗しており、production 実装待ち。
- Size: small
- Dependencies: TRACKER-041
- Exit Criteria:
  - `TrackerConnectionLibThirdPartyTrackerTests.cs` を all tracker 保存 contract に合わせて rename / 更新し、self 除外期待を取り下げる
  - ibis 自身の `uuid` / `sourceName` と一致する packet も snapshot / observed tracker state として保持される failing test を追加する
  - external tracker packet も snapshot / observed tracker state として保持される contract を維持する
  - source role / label は保存除外条件ではなく表示・比較用 metadata であり、unknown でも保存を落とさない failing test を追加する
  - snapshot replay の前提として、保存 state から official packet payload を後で復元できる failing test を追加する
  - production implementation は変更せず、focused test が production 実装不足で失敗することを記録する
...
| TRACKER-042 | 全 tracker 保存 contract へテストを修正する | comparison-logging | in_progress | TRACKER-041 | `TrackerConnectionLibThirdPartyTrackerTests.cs` を `TrackerConnectionLibAllTrackerSnapshotContractTests.cs` へ rename し、self 除外期待から all tracker 保存期待へ修正済み。ibis 自身の packet と external tracker packet が同時に保持されること、source role / label / metadata が保存後の表示・比較用情報として扱われること、判別不能な packet でも保存を落とさないこと、snapshot replay の前提として official packet payload を復元できることを focused failing test で固定済み。production implementation は未変更で、focused test は self 除外と `SourceRole` 未実装により失敗中。作業レポートは `reports/tracker-042-all-trackers-tdd-contract-20260512112546.md`。 |
