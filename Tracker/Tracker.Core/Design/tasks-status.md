...
- ID: TRACKER-041
- Title: 全 tracker packet 保存方針へ設計と tracking を修正する
- Phase: comparison-logging
- Status: in_progress
- TDD Entry: 既存の `TrackerConnectionLibThirdPartyTrackerTests.cs` と production 実装は self 除外を契約化しており、新方針と矛盾する。次作業で test contract を all tracker 保存へ修正する。
- Size: small
...
- Exit Criteria:
  ...
  - `tracker-server-cli-ui-detail-design.md` と `tracker-architecture-plan.md` から保存要件としての self 除外を取り下げる
  - 見えている `TrackerWrapperPacket` はすべて tracker packet snapshot sidecar へ保存し、ibis 自身の official packet と詳細ログの重複保持を許容する設計へ更新する
  - self / 3rdparty / unknown の判別は保存除外ではなく、後続表示・比較用の role / label / metadata として扱う方針を明記する
  - 3rdparty tracker snapshot を session folder 内に保持し、`Tracker.CaptureReplay` / diagnostics / playback が後で再生・比較表示できる設計へ更新する
  - tracking 上で、次作業が self 除外 test / 実装の review ではなく、all tracker 保存 contract への test 修正であることを明記する
...
| TRACKER-041 | 全 tracker packet 保存方針へ設計と tracking を修正する | comparison-logging | in_progress | TRACKER-040 | 追加方針により、既存の self 除外前提を取り下げる。見えている `TrackerWrapperPacket` はすべて snapshot sidecar へ保存し、ibis 自身の official packet と詳細ログの重複保持を許容する。self / 3rdparty / unknown の判別は保存除外ではなく後続表示・比較用の role / label / metadata として扱い、判別不能でも保存を落とさない。3rdparty tracker snapshot は raw payload 参照または復元可能情報、source uuid / sourceName / remote endpoint、receivedAt、tracked frame number / timestamp、summary を持ち、`Tracker.CaptureReplay` / diagnostics / playback が session folder 内の snapshot log から再生・比較表示できる設計へ更新する。現 self 除外 test / 実装は新方針と矛盾するため、次作業は test contract を all tracker 保存へ修正すること。 |
| TRACKER-042 | 全 tracker 保存 contract へテストを修正する | comparison-logging | todo | TRACKER-041 | `TrackerConnectionLibThirdPartyTrackerTests` を self 除外期待から all tracker 保存期待へ修正し、ibis 自身の packet と 3rdparty packet が同時に保持されること、source role / label / metadata が保存後の表示・比較用情報として扱われること、判別不能な packet でも保存を落とさないことを failing test で固定する。 |
