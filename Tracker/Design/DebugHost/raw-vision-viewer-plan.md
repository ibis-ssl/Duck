...
- パケットの永続化はしない
- 未加工の検出情報を超える追跡処理、状態推定処理、競技場全体の状態表現の解釈は入れない
...
[^live-store]: ライブ表示で使う状態保存先: 通常の映像画面の現在表示を更新するための保存先。CaptureOn の記録単位を保存する `TrackerPacketSnapshotLogWriter` や補助ファイルの書き込み処理は、この役割に使わない。
