...
- 段階: 文書検査整備
- 状態: 承認済み本文修正と許可語登録は PR #20 へ公開済み。全範囲の文書検査を終了コード 0 にする作業を継続中。
- 規模: 中
- 依存関係: `DOC-LINT-002`、承認済み許可語、共有文書検査器の修正。
- 完了条件:
  - コード上の名称、実際の表示名、設定名など英語で保持すべき表記と、通常の説明文を区別し、説明文の不要な英語を読みやすい日本語へ直す。
  ...
  - `npm run lint:md` を全対象文書に対して実行し、終了コード 0 を確認する。
  - 最終 PR の先頭コミットと同じ `head_sha` の CI を確認し、成功結果と検証証跡を記録する。
...
- 追加確認: 必須修正の指摘なし。承認済み説明文の未登録語 7 件と、全範囲の綴り検査 1,029 件は残課題として保持する。

- 2026-09-15 の追加依頼: コード上の名称や実際の表示名を残し、それ以外の設計上の説明語を日本語へ寄せる。読みやすさを優先し、分かりにくい片仮名への一括置換は行わない。
- 本文修正範囲: 基本設計、エンジン詳細、テスト設計、診断画面詳細、診断実行体の保守設計、映像表示設計、通常実行体設計の 7 文書。旧履歴、取り込み済み外部資料、製品コードは変更しない。
- 本文修正の公開先: `0f52e9e024c101cb46f9bb4d88fd4e066a477ec2`。文書単位で 7 コミットに分けて反映した。
