...
- 段階: 文書検査整備
- 状態: レビュー指摘への対応中。本文修正と履歴原文の保全を実施したが、許可語の全出現箇所の照合と引用中の未登録語の扱いが残る。完了扱いにはしない。
- 規模: 中
...
- 残課題: 許可語の出現単位照合、原文引用内の未登録語、修正後の再レビュー。最新の検証結果は `reports/markdown-wording-followup-20260916.md` と PR コメントに記録する。
...
- 共有検査器: 長文分割、脚注、語境界、日本語隣接英語、対象列挙の修正は 共有検査器の上流 PR #81 (`279b8da9ffe954f78fd8461555be429f1bdcc100`) として公開済み。

- 2026-09-15 の追加依頼: ソースコードに現れる名称や実際の表示名を残し、それ以外の設計上の説明語を日本語へ寄せる。読みやすさを優先し、分かりにくい片仮名への一括置換は行わない。
- 本文修正範囲: 基本設計、追跡処理詳細、テスト設計、診断画面詳細、診断実行体の保守設計、映像表示設計、通常実行体設計の 7 文書。旧履歴、取り込み済み外部資料、製品のソースコードは変更しない。
- 本文修正の公開先: `0f52e9e024c101cb46f9bb4d88fd4e066a477ec2`。文書単位で 7 回に分けて反映した。
- 本文修正の検証: 7 文書の `textlint` と差分検査は成功。`cspell` の残り 109 件は脚注参照名 106 件と `Unix` / `Base64` に由来する 3 件。許可一覧や検査除外設定を緩和していない。
- 本文修正の証跡: `reports/design-prose-japanese-20260915142353.md`、`reports/design-prose-validation-20260915142353.json`。全範囲検査の未登録語、脚注参照名の扱い、共有検査器の長文制限は残課題として区別する。

- 2026-09-17 の引き継ぎ確認: 診断画面の詳細設計について差分90件の本文と原文を照合し、採否と段落の対応を記録した。残り5文書と、4,336出現箇所の最終照合は継続中。全件確認済みや作業完了とはしない。
- 2026-09-17 の検証: 診断画面の詳細設計の対象検査3種と差分検査は成功。全体の `npm run lint:md` は終了値123。`feedback-points/feedback-points.md` の12・13行目にある2つの利用者発言の引用内の違反は、承認済みの引用例外を共有検査器へ実装する別作業として残る。
- 2026-09-17 の証跡: `reports/task-doc-lint-003-diagnostic-handoff-20260917064359.md`。作業担当の自己点検であり、独立最終レビューは未実施。
...
- 完了条件:
  - SudachiPy と辞書の Python 依存物を保存庫内の環境構築メモに残す。
...
- 実装証跡:
  ...
  - `lint:md:whitelist` は SudachiPy 版へ切り替えた。漢字語検出により現行許可一覧では意図的に失敗する。
- 確認証跡:
