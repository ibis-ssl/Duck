...
- 段階: 文書検査整備
- 状態: 承認済み本文修正と許可語登録を PR #20 へ公開し、全範囲の文書検査は終了値 0 を確認済み。最終 `current HEAD` の CI 確認待ち。
- 規模: 中
...
- 残課題: Duck 側の未登録語は 0 件。共有検査器の上流 PR #81 は公開済み・未統合で、PR #20 の最終 `current HEAD` CI 確認を残す。
...
- 2026-09-16 の全範囲検証: `npm run lint:md` は 19 文書を対象に終了値 0。`cspell` は 0 件、許可一覧違反は 0 件、`textlint` も成功した。
