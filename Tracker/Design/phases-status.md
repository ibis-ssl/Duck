...
- 現在の段階: 文書検査整備の Duck 側作業完了、PR #20 公開中
- 現在の作業: `DOC-LINT-003`
- 残り段階: 共有検査器 PR #81 の上流統合 / PR #20 の統合
- 再開証跡: `reports/doc-lint-resume-20260915.md`
- 本文修正: 現行設計書 7 文書を文書単位の 7 回の変更で PR #20 へ反映済み。製品のソースコードと許可一覧は、この本文修正では変更していない。
- 本文修正の検証: 2026-09-16 に全 19 文書の `npm run lint:md` が終了値 0。`cspell` 0 件、許可一覧違反 0 件、`textlint` 成功。詳細は `reports/markdown-lint-completion-20260916.md` を参照する。
