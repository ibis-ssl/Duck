...
`TRACKER-033` では名前空間を `Tracker.Core` のまま維持し、同一アセンブリ内のソースファイルの分割だけを行う。公開する型名、メンバー名、アクセス範囲、`null` を許可する箇所は変更しない。
...
```csharp
/// <summary>
/// 未加工の映像入力パケットから確定済みの追跡フレームとイベントを生成する、追跡エンジンの契約。
/// </summary>
```
...
- `RuntimeOverrides` が設定組のスナップショットに対する一時上書きであること
...
単純な値の取得処理、数式そのものが明らかな非公開の補助処理、1 行のラッパーには無理に付けない。ただし「なぜこの順序か」「どの挙動を固定するか」が読み手に伝わりにくい場合は、非公開のメソッドでも説明文または短い通常コメントを追加する。

### 通常コメント

通常コメントは複雑な処理のまとまりの前にだけ置く。既存コードをなぞるだけのコメントは追加しない。
...
- ロボットの向きの推定軸は、位置の mm 単位用の共分散を流用せず、rad 単位に対応する観測・予測それぞれの分散と角速度の制限を使う。設定組の Kalman の倍率に関する設定は、既定値との比で rad 用の基準値へ反映する。
