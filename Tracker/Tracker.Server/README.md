...
- Playback controls は従来どおり Play、Fast Forward、Stop の icon button 配置です。速度選択側には `等倍速`、`4x`、`16x`、`64x` の compact tabs を表示します。`等倍速` は Play として全 tick を逐次描画せず、30fps相当の表示更新で wall-clock 経過時間に対応する latest replay tick へ追従します。`4x` / `16x` / `64x` は調査用 Fast Forward として Play とは分離され、tick を間引かず capture-time delta と倍率で進みます。
