...
- Play / Fast Forward / scrub は diagnostics entry count ではなく、capture-time `ReceivedAt` に基づく unified replay timeline を使います。Vision / render より ER-FORCE などの tracker source が速い場合、timeline は fast tracker ticks を含み、Vision / render は latest-before frame を保持します。調査用の Fast Forward は speed selector で `4x` / `16x` / `64x` を選べます。
...
4. `/diagnostics` を開き、同じ session folder の `*.tracker-diagnostics.log` を選びます。unified replay timeline、scrubber、Play / Fast Forward、Fast Forward speed、左右 Field source selector、`Settings` modal の resolved settings を確認します。ER-FORCE など tracker source が Vision より速い capture では、timeline が fast tracker ticks を含み、Vision / render Field が latest-before frame を保持することも確認します。
