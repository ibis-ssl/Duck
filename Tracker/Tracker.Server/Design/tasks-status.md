...
- `RAW-VISION-013`: Vision split / overlay の source 候補、同時取得方針、geometry 基準、diagnostics 寄せ UI、固有名詞説明を設計へ固定する。
- `RAW-VISION-014`: Vision split / overlay の view-state、layout、source selection、overlay layer contract を TDD で固定する。
- `RAW-VISION-015`: Vision split / overlay UI と live source snapshot 接続を実装する。
...
| RAW-VISION-013 | Issue #10 Vision split / overlay の同時取得方針と設計を確定する | design | in-progress | RAW-VISION-012 | Source 候補を `Raw Aggregate`、`Raw Camera`、`Tracked`、`3rd party tracker` として固定し、live 比較で同時取得または同一 render tick snapshot をどう扱うか調査・設計する。geometry 基準、diagnostics 寄せ UI、固有名詞説明、採用/不採用方針を `raw-vision-viewer-plan.md` と report に記録し、PR #15 へ push する。 |
| RAW-VISION-014 | Vision split / overlay の TDD contract を追加する | verification | pending | RAW-VISION-013 | split / overlay mode、source selection、3rd party tracker source、same tick snapshot、geometry fallback、Layer A/B visibility、diagnostics 寄せ legend/details の期待挙動を単体テストで先に固定し、少なくとも 1 件の failing test を確認する。 |
| RAW-VISION-015 | Vision split / overlay UI と live source snapshot 接続を実装する | implementation | pending | RAW-VISION-014 | Vision 画面で左右分割と overlay mode を切り替えられ、Layer A/B source と visibility を diagnostics に近い UI で操作できる。Raw / Tracked / 3rd party tracker の同一 render tick snapshot を field に描画し、既存 raw/tracked 単体表示を壊さない。 |
