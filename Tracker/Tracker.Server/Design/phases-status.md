...
| verification | pending | Issue #10 split / overlay contract tests pass, focused validation passes, and `dotnet build Tracker/Tracker.Server/Tracker.Server.csproj` passes. |
...
| design | in-progress | `raw-vision-viewer-plan.md` records Issue #10 source options, same-tick acquisition strategy, diagnostics replay time-sync audit result, geometry policy, diagnostics-aligned split / overlay UI behavior, rejected alternatives, and glossary entries for named concepts before TDD begins. |
