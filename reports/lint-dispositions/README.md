...
Cは上限を広げたcspell診断1,619件、Wは長文分割による許可一覧の補助診断22,490件。同じ問題を重複して数える。全24,109診断記録を、正規化語と対応内容が同じ1,650項目へ整理した。各表の件数は全出現、場所は代表1件。一般漢字語は共通方針を参照する。日本語のW診断は正式な全体検査成功の代用ではない。
...
| [一般漢字語の件数・代表箇所](japanese.txt) | 1,053 | 本文維持と検査契約の見直し案。[語の一覧](japanese-index.txt)も参照できる。 |
| [その他の対応](other-actions.txt) | 142 | 識別子、脚注ID、技術表記、誤分割、日本語の推敲。 |

項目数は表記違い・対応違いをまとめた数で、異なる不具合の件数ではない。その他の対応では先頭列に分類を付けている。[代表箇所の訂正](reference-corrections.txt)も併せて参照する。
...
許可候補や一般漢字語の検査契約変更は未適用。候補の提示を承認や修正完了とは扱わない。通常の日本語へ寄せる場合も、読みやすさと仕様・過去の検証結果の維持を優先する。文脈で意味が変わる語は、単語表だけによる自動一括置換を行わない。
...
元入力、全検出箇所、再分類済みの24,109件とコマンド出力はRDC端末の `/home/ibis/.local/share/duck-doc-lint/lint-plan-publication-20260915T164549` と、元診断の `lint-disposition-20260915T154427` に保持する。公開した一覧は全語の対応表であり、全検出箇所を1件ずつ並べた原本とは区別する。
...
RDCで元の診断記録と再分類後の記録IDを比較し、24,109件が維持され、未分類が0件であることを確認した。5つの表の集計は1,650項目、C=1,619件、W=22,490件で元の診断と一致した。これは対応案の網羅性の検証であり、各出現箇所の本文修正を完了したという意味ではない。

通常の検査対象を再列挙して20文書、レポート0文書を確認した。`npm run lint:md` の再実行は終了コード123で未通過。標準出力・標準エラー・終了コードと網羅性の検証結果は、上記の保存先の `published-full-lint.*` と `final-completeness-validation.json` に保存した。CIは公開後のPR HEADとrunのhead SHAを照合して別途記録し、マージは実施しない。
