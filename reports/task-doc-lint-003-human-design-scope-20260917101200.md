...
| CLI-UI | `Tracker/Design/DebugHost/debug-host-cli-ui-detail-design.md` | 724 | 0 | 724 |
...
`tracker-core-engine-detail-design.md` は133/133出現の対応表が公開済みなので、新規担当の作業対象ではなく回帰確認用の参照とする。残り2,009件を4文書へ完全に分割し、同じ原出現IDを複数担当で処理しない。
...
この範囲定義時点では、対象2,142件中133件が完了、**残り2,009件**。本文の追加修正はこの範囲定義では行っていない。今回の対象変更により、4,336件全体の未処理数をDOC-LINT-003の今回完了条件には用いない。
